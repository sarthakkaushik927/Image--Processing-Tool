...
export default function MagicBrushWorkspace({ setPage, onImageDownloaded }) {
    ...
    const API_BASE = import.meta.env.VITE_ML_API; // https://ccc.anurag11.me
    ...
    const handleRemoveBackground = async () => {
        ...
        const fileInput = document.getElementById("magic-upload");
        ...
        try {
            ...
            if (data.status === "success" && data.output_url) {
                // Construct full URL if output_url is relative
                const fullURL = data.output_url.startsWith("http")
                    ? data.output_url
            ...
            console.error(err);
            alert("Server error: Unable to process image.");
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleDownload = () => {
        ...
        if (onImageDownloaded) onImageDownloaded(processedImageURL, downloadName);
